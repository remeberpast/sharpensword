...
//	//排序趟数的循环
//	for (i_interval = 1;i_interval < sz;i_interval = 2 * i_interval)
//	{
//		//每趟排序选取区间的循环
...
void bucket_sort(int arr[], int sz)
{
	//用二维数组分桶
	int bucket[5][5] = { 0 };
	//每个桶里有多少个元素
...
//画图以助理解
void radixsort(int arr[], int sz, int exp)
{
	int i = 0;
	...
	for (i = sz - 1;i > 0;i--)
	{
		...
		result[i_exp - 1] = arr[i];
		buckets[i_exp]--;
	}
	//将已排序的数组result复制给arr,准备下一次排序
	//memset(arr, result, sz * sizeof(int));
}
